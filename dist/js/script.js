...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const modalSpinner = document.querySelector('[data-modal="spinner"]'); // timer for open modal
  ...
  new MenuCard('img/tabs/post.jpg', 'post', 'Меню "Постное"', 'Меню "Постное" - это тщательный подбор ингредиентов: полное отсутствие продуктов животного происхождения, молоко из миндаля, овса, кокоса или гречки, правильное количество белков за счет тофу и импортных вегетарианских стейков.', 17, '.menu__field .container', 'menu__item', 'test-class1').renderCard(); // forms
  ...
  function postData(form) {
    form.addEventListener('submit', e => {
      ...
      request.open('POST', '../server.php'); // RequestHeader dont need if we posting standard data from FormData
      ...
      const json = JSON.stringify(object); // request.send(formData);
